...
public class OCamlRecursiveClientCodegen extends DefaultCodegen implements CodegenConfig {
    private final Logger LOGGER = LoggerFactory.getLogger(OCamlRecursiveClientCodegen.class);
    public static final String PACKAGE_NAME = "packageName";
    public static final String PACKAGE_VERSION = "packageVersion";

    private static final String VENDOR_IS_ONE_OF = "x-ocaml-isOneOf";
    private static final String VENDOR_IS_ANY_OF = "x-ocaml-isAnyOf";
    private static final String VENDOR_RETURN_FREE_FORM_OBJECT = "x-return-free-form-object";
    private static final String VENDOR_RETURNTYPE_ENUM = "x-returntype-enum";
    private static final String VENDOR_RETURNTYPE_IS_ENUM = "x-returntype-is-enum";
    /**
     * Tag the first model because OCaml recursive types have the first type marked with `type` and the others with `and`.
     */
    private static final String VENDOR_IS_FIRST_MODEL = "x-ocaml-is-first-model";
    ...
    public String getHelp() {
        return "Generates an OCaml client library, with support for recursive types.";
    }

    public OCamlRecursiveClientCodegen() {
        super();
        ...
        outputFolder = "generated-code/ocaml-recursive";
        modelTemplateFiles.put("model.mustache", ".ml");

        // default HIDE_GENERATION_TIMESTAMP to true
        hideGenerationTimestamp = Boolean.TRUE;

        embeddedTemplateDir = templateDir = "ocaml-recursive";
    ...
    public Map<String, ModelsMap> postProcessAllModels(Map<String, ModelsMap> orgObjs) {
        final Map<String, ModelsMap> superobjs = super.postProcessAllModels(orgObjs);
        ...
        for (Map.Entry<String, ModelsMap> modelEntry : superobjs.entrySet()) {
            ...
            for (ModelMap mo : models) {
                ...
                if (!cm.oneOf.isEmpty()) {
                    ...
                    cm.getVendorExtensions().put(VENDOR_IS_ONE_OF, true);
                }
                if (!cm.anyOf.isEmpty()) {
                    // Add a boolean if it is a `anyOf`, because Mustache does not let us check if a list is non-empty
                    cm.getVendorExtensions().put(VENDOR_IS_ANY_OF, true);
                }
            }
        }
        ...
        return mergeAllModelsInOneFile(superobjs);
    }
...
//        return capitalize(toModelFilename(name));
    ...
    public OperationsMap postProcessOperationsWithModels(OperationsMap objs, List<ModelMap> allModels) {
        ...
        for (CodegenOperation operation : operations) {
            if ("Yojson.Safe.t".equals(operation.returnBaseType)) {
                operation.vendorExtensions.put(VENDOR_RETURN_FREE_FORM_OBJECT, true);
            }
            ...
            if (operation.returnType != null && operation.returnType.startsWith("Enums.")) {
                ...
                operation.vendorExtensions.put(VENDOR_RETURNTYPE_ENUM, returnTypeEnum);
                operation.vendorExtensions.put(VENDOR_RETURNTYPE_IS_ENUM, true);
            }
        }
    ...
    /**
     * Merge all models into a single file, to support recursive types.
     * Shamelessly copied and adapted from the Elm generator.
     * @see ElmClientCodegen
     */
    @SuppressWarnings({"static-method"})
    public Map<String, ModelsMap> mergeAllModelsInOneFile(final Map<String, ModelsMap> objs) {
        ...
        models.stream().findFirst().ifPresent(modelMap -> modelMap.getModel().getVendorExtensions().put(VENDOR_IS_FIRST_MODEL, true));
